...
class GameController extends Controller
{
    ...
    public function index(ListAssets $request): View
    {
        ...
        return view('game.index', ['assets' => $paginator]);
    }
    ...
    public function show(Asset $asset): View
    {
        ...
        return view('game.show', ['asset' => $asset]);
    }

    /**
     * Display the form used to submit an game.
     */
    public function create(): View
    {
        return view('game.create', ['editing' => false]);
    }
    ...
    public function store(SubmitAsset $request): RedirectResponse
    {
        ...
        // The user must be authenticated to submit an game.
        ...
        $author = Auth::user();
        Log::info("$author submitted the asset $game.");

        return redirect(route('game.show', $asset));
    }

    /**
     * Display the form used to edit an game.
     */
    ...
    /**
     * Store modifications to an existing game.
     */
    public function update(Asset $asset, SubmitAsset $request): RedirectResponse
    {
        ...
        $author = Auth::user();
        Log::info("$author updated the asset $game.");
    ...
    public function publish(Asset $asset, Request $request): RedirectResponse
    {
        ...
        $admin = Auth::user();
        Log::info("$admin unpublished $game.");

        return redirect(route('game.show', $asset));
    }

    /**
     * Unpublishes an game. This can only be done by an administrator.
    ...
    public function updateReview(AssetReview $assetReview, SubmitReview $request): RedirectResponse
    {
        ...
        if ($assetReview->author && $asset && $user) {
            if ($assetReview->author_id === $user->id) {
                ...
                Log::info("$user edited their review for $game.");
            } else {
                $request->session()->flash('statusType', 'success');
                $request->session()->flash(
                    'status',
                ...
                Log::info("$user edited $assetReview->author's review for $game.");
            }
        }
    ...
    public function destroyReview(AssetReview $assetReview, Request $request): RedirectResponse
    {
        ...
        if ($assetReview->author && $asset && $user && $author) {
            if ($user->is_admin && $assetReview->author->id !== $user->id) {
                ...
                Log::info("$user removed $author's review for $game.");
            } else {
                $request->session()->flash(
                    'status',
                ...
                Log::info("$user removed their review for $game.");
            }
        }
    ...
    public function storeReviewReply(AssetReview $assetReview, SubmitReviewReply $request): RedirectResponse
    {
        ...
        if ($assetReview->author) {
            ...
            $author = Auth::user();
            Log::info("$author replied to $assetReview->author's review for $assetReview->game.");
        }

        return redirect(route('game.show', $assetReview->asset));
    }
}
